...
# Conservative Stage 1.2 continuation:
# resume from the best Stage 1 v4 delta checkpoint, but keep training only
# the HRS ME-reference delta head. Do not open hist_fusion/safe scope yet.
...
SAVE_DIR="results/checkpoints_stage1_2_hr_v4_delta"
LOG_DIR="results/logs_stage1_2_hr_v4_delta"
LOG_FILE="${LOG_DIR}/train_stage1_2_hr_v4_delta_$(date +%Y%m%d_%H%M%S).txt"

mkdir -p "$SAVE_DIR" "$LOG_DIR"

echo "Starting NVC-FM-HR Stage 1.2 conservative delta-only training..."
...
echo "Stage 1.2 HRS conservative delta-only training finished."
