...
def parse_args():
    ...
    parser.add_argument('--model_type', type=str, default='dmc',
                        choices=['dmc', 'dmc_hr', 'wrapper'])
    parser.add_argument('--me_delta_scale', type=float, default=0.02)
    parser.add_argument('--wrapper_use_pre', type=str2bool, default=True)
    parser.add_argument('--wrapper_use_post', type=str2bool, default=True)
    parser.add_argument('--wrapper_post_as_ref', type=str2bool, default=True)
...
def build_p_frame_model(model_type, ec_thread=False, stream_part=1, inplace=True,
                        me_delta_scale=0.02, wrapper_use_pre=True,
                        wrapper_use_post=True, wrapper_post_as_ref=True):
    if model_type == 'dmc':
        return DMC(ec_thread=ec_thread, stream_part=stream_part, inplace=inplace)
    if model_type == 'dmc_hr':
        try:
            model = DMC_HR(ec_thread=ec_thread, stream_part=stream_part, inplace=inplace,
    ...
    if model_type == 'wrapper':
        try:
            core = DMC_HR(ec_thread=ec_thread, stream_part=stream_part, inplace=inplace,
                          me_delta_scale=me_delta_scale)
        except TypeError as exc:
            if "me_delta_scale" not in str(exc):
                raise
            core = DMC_HR(ec_thread=ec_thread, stream_part=stream_part, inplace=inplace)
        set_me_delta_scale(core, me_delta_scale)
        return NeuralWrapper(
            core,
            qp_num=DMC.get_qp_num(),
            inplace=inplace,
            use_pre=wrapper_use_pre,
            use_post=wrapper_use_post,
            post_as_ref=wrapper_post_as_ref,
        )
...
def load_p_frame_state(model, state_dict, model_type):
    if model_type == 'dmc':
        model.load_state_dict(state_dict)
        return
    if model_type == 'wrapper' and any(key.startswith(('core.', 'pre.', 'post.'))
                                       for key in state_dict):
        missing, unexpected = model.load_state_dict(state_dict, strict=False)
        if unexpected:
            print(f"Unexpected keys when loading wrapper: {unexpected}")
        if missing:
            print(f"Missing keys when loading wrapper: {missing}")
        return
    target = model.core if model_type == 'wrapper' else model
    missing, unexpected = target.load_state_dict(state_dict, strict=False)
    if unexpected:
        print(f"Unexpected keys when loading {model_type}: {unexpected}")
    ...
    if non_hr_missing:
        print(f"Missing non-HRS keys when loading {model_type}: {non_hr_missing}")
    if hr_missing:
        print(f"Initialized {len(hr_missing)} HRS keys from scratch for {model_type}.")
    ...
    with torch.no_grad():
        for frame_idx in range(frame_num):
            ...
            if frame_idx % args['intra_period'] == 0:
                ...
                dpb = result["dpb"]
                recon_frame = result.get("display_frame", dpb["ref_frame"])
    ...
    with torch.no_grad():
        while decoded_frame_number < frame_num:
            ...
            if header['nal_type'] == NalType.NAL_I:
                ...
                dpb = decoded["dpb"]
                recon_frame = decoded.get("display_frame", dpb["ref_frame"])
...
def init_func(args, gpu_num):
    ...
    if not args.force_intra:
        ...
        p_frame_net = build_p_frame_model(args.model_type, args.ec_thread,
                                          args.stream_part_p, inplace=True,
                                          me_delta_scale=args.me_delta_scale,
                                          wrapper_use_pre=args.wrapper_use_pre,
                                          wrapper_use_post=args.wrapper_use_post,
                                          wrapper_post_as_ref=args.wrapper_post_as_ref)
        load_p_frame_state(p_frame_net, p_state_dict, args.model_type)
