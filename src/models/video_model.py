...
from ..nvcx.hierarchy import HierarchicalContextModulation, ReferenceState
...
class DMC_HR(DMC):
    def __init__(
        ...
        super().__init__(ec_thread=ec_thread, stream_part=stream_part, inplace=inplace)
        self.hierarchy_size = hierarchy_size
        self.use_ste_quant = True
        self.me_delta_scale = me_delta_scale
        self.ref_hierarchy = HierarchicalContextModulation(
            channels=(g_ch_1x, g_ch_2x, g_ch_4x),
            num_experts=hierarchy_experts,
            qp_num=self.get_qp_num(),
            me_delta_scale=me_delta_scale,
            max_alpha_hist=max_alpha_hist,
            max_alpha_expert=max_alpha_expert,
            gate_init=hrs_gate_init,
            inplace=inplace,
        )

    def quant(self, x):
        if self.training and self.use_ste_quant:
            return x + (torch.round(x) - x).detach()
        return super().quant(x)

    def _dpb_without_hierarchy(self, dpb):
        return {
            "ref_frame": dpb["ref_frame"],
            "ref_feature": dpb.get("ref_feature", None),
            "ref_mv_feature": dpb.get("ref_mv_feature", None),
            "ref_y": dpb.get("ref_y", None),
            "ref_mv_y": dpb.get("ref_mv_y", None),
        }

    @staticmethod
    def _scalar_index(value, default=0):
        if value is None:
            return default
    ...
    def _feature_fa_idx(self, fa_idx):
        fa_idx = self._scalar_index(fa_idx)
        if fa_idx >= len(self.feature_adaptor):
            return 0
        return fa_idx

    def _make_reference_state(self, dpb, q_index=0, fa_idx=0, age=0):
        fa_idx_safe = self._feature_fa_idx(fa_idx)
        ref_feature1, ref_feature2, ref_feature3 = self.multi_scale_feature_extractor(
            self._dpb_without_hierarchy(dpb), fa_idx_safe
        )
        return ReferenceState(
            feat1=ref_feature1,
            feat2=ref_feature2,
            feat3=ref_feature3,
            lowrank3=dpb["ref_frame"],
            q_index=self._scalar_index(q_index),
            fa_idx=fa_idx_safe,
            age=age,
        )

    def get_reference_states(self, dpb, q_index=0, fa_idx=0):
        states = []
        for age, state in enumerate(dpb.get("ref_hierarchy", [])[1:], start=1):
            states.append(ReferenceState(
                feat1=state.feat1,
                feat2=state.feat2,
                feat3=state.feat3,
                lowrank3=state.lowrank3,
                q_index=state.q_index,
                fa_idx=state.fa_idx,
                age=age,
            ))
        states.insert(0, self._make_reference_state(dpb, q_index, fa_idx, age=0))
        return states[:self.hierarchy_size]

    def update_reference_bank(self, dpb, feature, x_hat, q_index=0, fa_idx=0):
        fa_idx_safe = self._feature_fa_idx(fa_idx)
        ref_feature1, ref_feature2, ref_feature3 = self.feature_extractor(feature)
        new_state = ReferenceState(
            feat1=ref_feature1.detach(),
            feat2=ref_feature2.detach(),
            feat3=ref_feature3.detach(),
            lowrank3=x_hat.detach(),
            q_index=self._scalar_index(q_index),
            fa_idx=fa_idx_safe,
            age=0,
        )
        bank = [new_state]
        for age, state in enumerate(dpb.get("ref_hierarchy", []), start=1):
            if len(bank) >= self.hierarchy_size:
                break
            bank.append(ReferenceState(
                feat1=state.feat1.detach(),
                feat2=state.feat2.detach(),
                feat3=state.feat3.detach(),
                lowrank3=state.lowrank3.detach(),
                q_index=state.q_index,
                fa_idx=state.fa_idx,
                age=age,
            ))
    ...
    def get_hierarchy_output(self, dpb, q_index=0, fa_idx=0, mode="encode", route_id=None):
        states = self.get_reference_states(dpb, q_index, fa_idx)
        return self.ref_hierarchy(states, q_index=q_index, fa_idx=fa_idx, mode=mode, route_id=route_id)

    def hier_motion_compensation(self, dpb, mv, fa_idx, hierarchy_output):
        warpframe = block_mc_func(dpb["ref_frame"], mv)
        mv2 = bilineardownsacling(mv) / 2
        mv3 = bilineardownsacling(mv2) / 2
        ref_feature1, ref_feature2, ref_feature3 = hierarchy_output.ref_pyramid
        context1_init = block_mc_func(ref_feature1, mv)
        context1 = self.align(ref_feature1, torch.cat(
            (context1_init, warpframe, mv), dim=1), mv)
        context2 = block_mc_func(ref_feature2, mv2)
    ...
    def motion_estimation_and_mv_encoding(self, x, dpb, mv_y_q_enc, q_index=0, fa_idx=0):
        hierarchy_output = self.get_hierarchy_output(dpb, q_index, fa_idx, mode="encode")
        est_mv = self.optic_flow(x, hierarchy_output.me_ref)
        ref_mv_feature = dpb["ref_mv_feature"]
        mv_y = self.mv_encoder(est_mv, ref_mv_feature, mv_y_q_enc)
        return mv_y, hierarchy_output

    def _make_result_dpb(self, old_dpb, x_hat, feature, mv_feature, y_hat, mv_y_hat, q_index, fa_idx):
        return {
            "ref_frame": x_hat,
            "ref_feature": feature,
            "ref_mv_feature": mv_feature,
            "ref_y": y_hat,
            "ref_mv_y": mv_y_hat,
            "ref_hierarchy": self.update_reference_bank(old_dpb, feature, x_hat, q_index, fa_idx),
        }

    def compress(self, x, dpb, q_index, fa_idx):
        mv_y_q_enc, mv_y_q_dec, y_q_enc, y_q_dec = self.get_all_q(q_index)
        mv_y, hierarchy_output = self.motion_estimation_and_mv_encoding(
            x, dpb, mv_y_q_enc, q_index, fa_idx
        )
        mv_y_pad, slice_shape = self.pad_for_y(mv_y)
        mv_z = self.mv_hyper_prior_encoder(mv_y_pad)
        mv_z_hat = torch.round(mv_z)
        ...
        mv_hat, mv_feature = self.mv_decoder(mv_y_hat, mv_y_q_dec)
        context1, context2, context3, warpframe = self.hier_motion_compensation(
            dpb, mv_hat, fa_idx, hierarchy_output
        )
        ...
        result = {
            "dpb": self._make_result_dpb(
                dpb, x_hat, feature, mv_feature, y_hat, mv_y_hat, q_index, fa_idx
            ),
            "bit_stream": bit_stream,
            "route_info": {
                "route_topk": hierarchy_output.route_topk,
                "balance_loss": hierarchy_output.balance_loss,
            },
        }
        return result
    ...
    def decompress(self, bit_stream, dpb, sps):
        ...
        mv_z_hat = self.bit_estimator_z_mv.decode_stream(z_size, dtype, device, 0)
        z_hat = self.bit_estimator_z.decode_stream(z_size, dtype, device, 0)
        hierarchy_output = self.get_hierarchy_output(
            dpb, q_index=sps['qp'], fa_idx=sps['fa_idx'], mode="decode"
        )
        mv_params = self.mv_prior_param_decoder(mv_z_hat, dpb, slice_shape)
        mv_y_hat = self.decompress_four_part_prior(mv_params,
                                                   self.mv_y_spatial_prior_adaptor_1,
        ...
        mv_hat, mv_feature = self.mv_decoder(mv_y_hat, mv_y_q_dec)
        context1, context2, context3, _ = self.hier_motion_compensation(
            dpb, mv_hat, sps['fa_idx'], hierarchy_output
        )
        ...
        return {
            "dpb": self._make_result_dpb(
                dpb, x_hat, feature, mv_feature, y_hat, mv_y_hat, sps['qp'], sps['fa_idx']
            ),
            "decoding_time": t1 - t0,
        }
    ...
    def forward_one_frame(self, x, dpb, q_index=None, fa_idx=0):
        ...
        result = {
            ...
            "aux": {
                "warpframe": warpframe,
                "me_ref": hierarchy_output.me_ref,
                "hrs_me_delta_reg": hierarchy_output.me_delta_reg,
                "hrs_feature_reg": hierarchy_output.feature_reg,
                "hrs_expert_reg": hierarchy_output.expert_reg,
                "hrs_gate_values": hierarchy_output.gate_values,
            },
            "route_info": {
                "route_logits": hierarchy_output.route_logits,
                "route_topk": hierarchy_output.route_topk,
                "balance_loss": hierarchy_output.balance_loss,
            },
        }
        if hierarchy_output.balance_loss is not None:
            result["balance_loss"] = hierarchy_output.balance_loss
        return result
