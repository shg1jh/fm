...
CKPT_DIR="results/checkpoints_stage1b_soft_from_v4_step2000"
OUT_DIR="results/eval_stage1b_soft_from_v4_step2000"
LOG_DIR="results/logs_eval_stage1b_soft_from_v4_step2000"

mkdir -p "$OUT_DIR" "$LOG_DIR"

echo "Starting Stage 1B soft checkpoint evaluation..."
...
run_eval() {
  local tag="$1"
  local checkpoint="$2"
  local output_json="$OUT_DIR/eval_stage1b_soft_${tag}_pframes.json"
  local log_file="$LOG_DIR/eval_stage1b_soft_${tag}_$(date +%Y%m%d_%H%M%S).txt"
...
run_eval "step2100" "$CKPT_DIR/stage1_hr_step2100.pth.tar"
run_eval "step2200" "$CKPT_DIR/stage1_hr_step2200.pth.tar"

echo "All Stage 1B soft checkpoint evaluations finished."
