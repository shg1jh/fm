...
CKPT_DIR="results/checkpoints_stage1_2_hr_v4_safe"
OUT_DIR="results/eval_stage1_2_hr_v4_safe"
LOG_DIR="results/logs_eval_stage1_2_hr_v4_safe"

mkdir -p "$OUT_DIR" "$LOG_DIR"

echo "Starting Stage 1.2 safe checkpoint evaluation..."
...
run_eval() {
  local tag="$1"
  local checkpoint="$2"
  local output_json="$OUT_DIR/eval_stage1_2_hr_v4_safe_${tag}_pframes.json"
  local log_file="$LOG_DIR/eval_stage1_2_hr_v4_safe_${tag}_$(date +%Y%m%d_%H%M%S).txt"
...
echo "All Stage 1.2 safe checkpoint evaluations finished."
