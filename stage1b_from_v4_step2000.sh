...
# Stage 1B must start from the stable Stage 1 v4 delta checkpoint.
RESUME_CHECKPOINT="results/checkpoints_stage1_hr_v4_delta/stage1_hr_step2000.pth.tar"

# Stage 1B soft probe outputs.
SAVE_DIR="results/checkpoints_stage1b_soft_from_v4_step2000"
LOG_DIR="results/logs_stage1b_soft_from_v4_step2000"
LOG_FILE="${LOG_DIR}/train_stage1b_soft_from_v4_step2000_$(date +%Y%m%d_%H%M%S).txt"

mkdir -p "$SAVE_DIR" "$LOG_DIR"

echo "Starting NVC-FM-HR Stage 1B soft probe from v4 step2000..."
...
echo "Stage 1B soft probe finished."
