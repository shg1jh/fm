...
CKPT_DIR="results/checkpoints_stage1b_soft_continue_from_step2200"
OUT_DIR="results/eval_stage1b_soft_continue_from_step2200"
LOG_DIR="results/logs_eval_stage1b_soft_continue_from_step2200"

mkdir -p "$OUT_DIR" "$LOG_DIR"

echo "Starting Stage 1B soft continuation checkpoint evaluation..."
...
run_eval() {
  local tag="$1"
  local checkpoint="$2"
  local output_json="$OUT_DIR/eval_stage1b_soft_continue_${tag}_pframes.json"
  local log_file="$LOG_DIR/eval_stage1b_soft_continue_${tag}_$(date +%Y%m%d_%H%M%S).txt"
...
run_eval "step2300" "$CKPT_DIR/stage1_hr_step2300.pth.tar"

echo "Stage 1B soft continuation checkpoint evaluation finished."
