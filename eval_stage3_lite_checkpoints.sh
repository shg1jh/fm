...
CKPT_DIR="results/checkpoints_stage3_lite_from_v4_step2000"
OUT_DIR="results/eval_stage3_lite_from_v4_step2000"
LOG_DIR="results/logs_eval_stage3_lite_from_v4_step2000"

mkdir -p "$OUT_DIR" "$LOG_DIR"

echo "Starting Stage 3-lite checkpoint evaluation..."
...
run_eval() {
  local tag="$1"
  local checkpoint="$2"
  local output_json="$OUT_DIR/eval_stage3_lite_${tag}_pframes.json"
  local log_file="$LOG_DIR/eval_stage3_lite_${tag}_$(date +%Y%m%d_%H%M%S).txt"
...
echo "All Stage 3-lite checkpoint evaluations finished."
