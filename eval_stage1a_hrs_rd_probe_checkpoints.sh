...
# Evaluate RD-aware Stage 1A HRS probe checkpoints.
...
CKPT_DIR="${CKPT_DIR:-results/checkpoints_stage1a_hrs_rd_probe_v3_from_official}"
OUT_DIR="${OUT_DIR:-results/eval_stage1a_hrs_rd_probe_v3_from_official}"
LOG_DIR="${LOG_DIR:-results/logs_eval_stage1a_hrs_rd_probe_v3_from_official}"
...
echo "Starting Stage 1A RD-aware HRS probe evaluation..."
...
for step in 250 500 750 1000; do
  checkpoint="$CKPT_DIR/stage1a_hrs_rd_step${step}.pth.tar"
  if [ -f "$checkpoint" ]; then
    run_eval "stage1a_hrs_rd_step${step}" "$checkpoint"
  else
    echo "Skip missing checkpoint: $checkpoint"
  fi
done

if [ -f "$CKPT_DIR/stage1a_hrs_rd_latest.pth.tar" ]; then
  run_eval "stage1a_hrs_rd_latest" "$CKPT_DIR/stage1a_hrs_rd_latest.pth.tar"
fi

echo "All Stage 1A RD-aware HRS probe evaluations finished."
