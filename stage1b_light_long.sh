...
# Stage 1B long training: HRS plus feature_adaptor/align/context_fusion/temporal_prior.
...
RESUME_CHECKPOINT="${RESUME_CHECKPOINT:-results/checkpoints_stage1a_hrs_long_from_official/stage1a_hrs_latest.pth.tar}"
SAVE_DIR="${SAVE_DIR:-results/checkpoints_stage1b_light_long_from_stage1a}"
LOG_DIR="${LOG_DIR:-results/logs_stage1b_light_long_from_stage1a}"
MAX_NEW_STEPS="${MAX_NEW_STEPS:-10000}"
SAVE_INTERVAL="${SAVE_INTERVAL:-1000}"
LOG_FILE="${LOG_DIR}/train_stage1b_light_long_$(date +%Y%m%d_%H%M%S).txt"

mkdir -p "$SAVE_DIR" "$LOG_DIR"

echo "Starting NVC-FM-HR Stage 1B light long training..."
...
python training/train_stage1b_light_long.py \
...
echo "Stage 1B light long training finished."
