...
CKPT_DIR="results/checkpoints_stage1_2_hr_v4_delta"
OUT_DIR="results/eval_stage1_2_hr_v4_delta"
LOG_DIR="results/logs_eval_stage1_2_hr_v4_delta"

mkdir -p "$OUT_DIR" "$LOG_DIR"

echo "Starting Stage 1.2 conservative delta checkpoint evaluation..."
...
run_eval() {
  local tag="$1"
  local checkpoint="$2"
  local output_json="$OUT_DIR/eval_stage1_2_hr_v4_delta_${tag}_pframes.json"
  local log_file="$LOG_DIR/eval_stage1_2_hr_v4_delta_${tag}_$(date +%Y%m%d_%H%M%S).txt"
...
run_eval "step2500" "$CKPT_DIR/stage1_hr_step2500.pth.tar"
run_eval "step3000" "$CKPT_DIR/stage1_hr_step3000.pth.tar"

echo "All Stage 1.2 conservative delta checkpoint evaluations finished."
